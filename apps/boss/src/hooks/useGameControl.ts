...
export function useGameControl() {
  ...
  const [sessionState, setSessionState] = useState<SessionCurrentResponse | null>(null);
  const [question, setQuestion] = useState<QuestionResponse | null>(null);
  const [ranking, setRanking] = useState<RankingLiveResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (cancelled) return;
      if (sessionRes.success) setSessionState(sessionRes.data);
      if (questionRes.success) setQuestion(questionRes.data);
  ...
  const start = useCallback(async (title: string, bossName: string) => {
    setBusy(true);
    setError(null);
  ...
  const resume = useCallback((existingSessionId: string) => {
    try {
