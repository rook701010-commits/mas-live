...
export function useOverlayState(sessionId: string | null) {
  const [state, setState] = useState<OverlayStateResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (sessionRes.success && sessionRes.data.status === "finished") {
        ...
        if (!cancelled && finalRes.success) setFinalResult(finalRes.data);
      }
    };
